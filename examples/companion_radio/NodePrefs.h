...
public:
  ...
  // [mishmesh] UI preferences. These remain at the tail of the legacy binary
  // layout and are also included in v1.17's serialized `comp` object below.
...
private:
  ...
  protected:
    void structure() override {
      ...
      // [mishmesh] Persist fork-specific settings in the new v1.17 format.
      def("sound_vol", _parent->sound_volume);
      def("sound_mask", _parent->sound_mute_mask);
      def("tone_ch", _parent->notify_tone_ch);
      def("tone_dm", _parent->notify_tone_dm);
      def("tz_qh", _parent->tz_quarter_hours);
      def("time_12h", _parent->time_fmt_12h);
      def("manual_time", _parent->manual_time_set);
      def("date_fmt", _parent->date_format);
      def("screen_sleep", _parent->screen_sleep);
      def("repeat_freq", _parent->repeat_saved_freq);
      def("ble_en", _parent->ble_enabled);
      def("full_notify", _parent->contacts_full_notify);
      def("tz_city", _parent->tz_city_index);
      def("onboarding", _parent->onboarding_state);
      def("brightness", _parent->screen_brightness);
      // [/mishmesh]
    }
  public:
